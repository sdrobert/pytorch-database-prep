...
def wsj_prepare_dict(data_root, dict_suffix=""):
    ...
    for line in cat(os.path.join(cmudict, "cmudict.0.7a")):
        ...
        if match is None:
            ...
            lexicon_raw_nosil_lines.append(" ".join([match.group(1), match.group(2)]))
...
def wsj_prepare_char_dict(data_root, src_dict_suffix, dst_dict_suffix):
    ...
    phn_lexicon2_raw_nosil_txt = os.path.join(phone_dir, "lexicon2_raw_nosil.txt")
    unique = OrderedDict()
    ...
    bad_chars = set('!~@#$%^&*()+=/",;:?_{}-')
    pipe_to(
        (" ".join([x] + [y for y in x if y not in bad_chars]) for x in unique.keys()),
    ...
    pipe_to(
        sort(set(cat(*(x.split(" ")[1:] for x in cat(char_lexicon2_raw_nosil_txt))))),
...
def dict_get_rules(
    in_,
    disallow_empty_suffix=True,
    min_prefix_len=3,
    ignore_prefix_stress=True,
    min_suffix_count=20,
):
    ...
    suffix_count = Counter(
        chain.from_iterable(
            (w[i:] for i in range(min_prefix_len, len(w) + 1)) for w in prons
        )
    )
    suffix_count = dict(
        (w, count) for (w, count) in suffix_count.items() if count >= min_suffix_count
    )
    ...
    suffix_pair_count = dict(
        (k, count)
        for (k, count) in suffix_pair_count.items()
...
def dict_get_candidate_prons(rules, dict_, words, min_prefix_len=3):
    ...
    prefixcount = Counter(
        chain.from_iterable((w[:p] for p in range(len(w) + 1)) for w in word2prons)
    )
        ...
        for prefix_len in range(min_prefix_len, len(word) + 1):
            ...
            for R in rules_array_ref:
                ...
                for base_pron in base_prons_ref:
                    ...
                    if (
                        base_pron_prefix_len < 0
                        or base_pron[base_pron_prefix_len:] != base_psuffix
                    ):
                        continue
                    ...
                    for destress in range(2):
                        ...
                        if rule_score is None:
                            continue
                        output = [word, pron, base_word, base_pron, rule, destress_mark]
                        if rule_score != -1:
...
def dict_select_candidate_prons(candidates, max_prons=4, min_rule_score=0.35):
    ...
    def process_word(cur_lines):
        ...
        for line in cur_lines:
            ...
            score = float(score)
            if score >= min_rule_score and score > pron2rule_score.get(pron, -1):
                pron2rule_score[pron] = score
                pron2line[pron] = line
        prons = sorted(
            pron2rule_score, key=lambda x: (-pron2rule_score[x], x), reverse=False
        )
...
def wsj_extend_dict(dir_13_32_1, data_root, src_dict_suffix, mincount=2):
    ...
    pipe_to(
        sort(set(cat(os.path.join(src_dict_dir, "lexicon1_raw_nosil.txt")))), dict_cmu
    )
    ...
    isword = set(x.strip() for x in cat(os.path.join(dst_dict_dir, "wordlist.cmu")))
    ...
    with open(os.path.join(dst_dict_dir, "unigrams"), "w") as unigrams, open(
        oov_counts_path, "w"
    ) as oov_cnts, open(oovlist_path, "w") as oov_lst:
    ...
    pipe_to(
        dict_get_acronym_prons(oovlist_path, dict_cmu), dict_acronyms_path,
    )
    ...
    pipe_to(
        (
            x
            for x in cat(os.path.join(dst_dict_dir, "oovlist"))
    ...
    pipe_to(
        (
            " ".join([w[::-1]] + p.split()[::-1])
            for (w, p) in (x.split(" ", 1) for x in cat(os.path.join(f_dir, "dict")))
        ),
        os.path.join(b_dir, "dict"),
    )

    pipe_to(
        (x[::-1] for x in cat(os.path.join(f_dir, "oovs"))), os.path.join(b_dir, "oovs")
    )
    ...
    for dir_ in (f_dir, b_dir):
        ...
        pipe_to(
            dict_limit_candidate_prons(
                hierarchy_path,
                dict_get_candidate_prons(rules_with_scores_path, dict_path, oovs_path),
            ),
            oov_candidates_path,
        )
    ...
    pipe_to(
        sorted(set(cat(dict_acronyms_path, dict_oovs_path))), dict_oovs_merged_path,
    )
...
def wsj_train_lms(data_root, src_dict_suffix, out_dir="local_lm", max_order=4):
    ...
    vocab = set(x.split()[0] for x in cat(os.path.join(src_dict_dir, "lexicon.txt")))
    vocab.remove("!SIL")
    ...
    sents = ngram_lm.text_to_sents(text, sent_end_expr=r"\n", word_delim_expr=r" +")
    del text
    ...
    for i, ngram_count in enumerate(ngram_counts[1:]):
        if i:
            to_prune.update(
                x for x in ngram_count if x[:-1] in to_prune or x[-1] in to_prune
            )
        else:
            to_prune.update(
                x for x in ngram_count if x[0] in to_prune or x[-1] in to_prune
            )
